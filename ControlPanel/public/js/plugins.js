...
function handlePluginPermissions(data) {
    ...
    // 插件能力（供大模型调用）
    html += '<div class="permissions-section">';
    html += '<h3>⚡ 插件能力（供大模型调用）</h3>';
    if (capabilities.length === 0) {
        html += '<p class="no-permissions">该插件没有声明可用能力</p>';
    } else {
        html += '<div class="capabilities-list">';
        capabilities.forEach(cap => {
            html += '<div class="capability-item">';
            html += `<div class="capability-header">`;
            html += `<span class="capability-name">${escapeHtml(cap.Name)}</span>`;
            html += `<span class="capability-method">${escapeHtml(cap.MethodName)}</span>`;
            html += `</div>`;
            html += `<div class="capability-description">${escapeHtml(cap.Description || '暂无描述')}</div>`;

            // 显示参数
            if (cap.Parameters && cap.Parameters.length > 0) {
                html += '<div class="capability-params">';
                html += '<span class="params-label">参数:</span>';
                html += '<ul class="params-list">';
                cap.Parameters.forEach(param => {
                    const requiredMark = param.Required ? '<span class="param-required">*</span>' : '';
                    const defaultValue = param.DefaultValue !== null && param.DefaultValue !== undefined
                        ? `<span class="param-default">默认: ${escapeHtml(String(param.DefaultValue))}</span>`
                        : '';
                    html += `<li class="param-item">`;
                    html += `<span class="param-name">${escapeHtml(param.Name)}${requiredMark}</span>`;
                    html += `<span class="param-type">(${escapeHtml(param.Type)})</span>`;
                    html += `<span class="param-desc">${escapeHtml(param.Description || '')}</span>`;
                    html += defaultValue;
                    html += `</li>`;
                });
                html += '</ul>';
