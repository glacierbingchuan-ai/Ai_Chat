...
12. 当用户聊暧昧语言时，可以跟着话题延申";
        ...
        private const string CHAT_HISTORY_PATH = "chat_history.json";
        private static List<ChatMessage> _chatHistory = new List<ChatMessage>();
        private static readonly object _chatHistoryLock = new object();
        ...
        private static void LoadChatHistoryFromDisk()
        {
            try
            {
                if (File.Exists(CHAT_HISTORY_PATH))
                {
                    string json = File.ReadAllText(CHAT_HISTORY_PATH);
                    ...
                    if (savedChatHistory != null && savedChatHistory.Count > 0)
                    {
                        ...
                        LogInfo("PERSISTENCE", $"Loaded {savedChatHistory.Count} chat messages from local disk");
                        return;
                    }
                }
                LogInfo("PERSISTENCE", "No chat history found or file is empty, initializing new chat history");
            }
            catch (Exception ex)
            {
                LogError("PERSISTENCE", "Failed to load chat history: " + ex.Message);
            }
        }

        private static void SaveChatHistoryToDisk()
        {
            try
            {
                lock (_chatHistoryLock)
                {
                    ...
                    File.WriteAllText(CHAT_HISTORY_PATH, json);
                }
            }
            catch (Exception ex)
            {
                LogError("PERSISTENCE", $"Failed to save chat history: {ex.Message}");
            }
        }

        private static void AddChatHistoryMessage(string role, string content = null, string meme = null)
        {
            string safeContent = content != null ? System.Net.WebUtility.HtmlEncode(content) : null;
            string safeMeme = meme != null ? System.Net.WebUtility.HtmlEncode(meme) : null;
            ...
            lock (_chatHistoryLock)
            {
                ...
                {
                    _chatHistory = _chatHistory.Skip(_chatHistory.Count - 1000).ToList();
                }
            }
        ...
        private static void BroadcastChatMessage(ChatMessage message)
        {
            BroadcastMessageToClients(new WebSocketMessage { Type = "chat_message", Data = message });
        }
        ...
        private static void BroadcastChatHistory()
        {
            ...
            lock (_chatHistoryLock)
            {
                chatHistory = _chatHistory.ToList();
            }
        ...
        private static async Task StartBotAsync()
        {
            while (!_globalCts.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(_controlPanelConfig.WebsocketToken))
                    {
                        currentWebSocket.Options.SetRequestHeader("Authorization", "Bearer " + _controlPanelConfig.WebsocketToken);
                    }

                    LogInfo("WS_CLIENT", "Attempting connection to WebSocket server: " + _controlPanelConfig.WebsocketServerUri);
                    await currentWebSocket.ConnectAsync(new Uri(_controlPanelConfig.WebsocketServerUri), _globalCts.Token);
                    ...
                    // 等待其中一个任务结束（通常是接收任务因为连接断开而结束）
                    await Task.WhenAny(receiveTask, keepAliveTask);

                    LogWarning("WS_CLIENT", "WebSocket connection lost or closed by server.");
                }
                catch (Exception ex)
                {
                    LogError("WS_CLIENT", "WebSocket connection failure.", ex);
                }
                finally
                {
                    // 清理旧连接
                    ...
                    //无论成功还是失败，重连前都强制等待 5 秒，防止刷屏
                    if (!_globalCts.IsCancellationRequested)
                    {
                        LogInfo("WS_CLIENT", "Waiting 5 seconds before reconnecting...");
        ...
        private static async Task CommitAndReplyAsync(string hid)
        {
            ...
            lock (_contextLock)
            {
                ...
                // Add to chat history
                AddChatHistoryMessage("user", finalizedMessage);
            }
        ...
        private static async Task SendAIRepliesStepByStep(List<dynamic> replyMsgs, CancellationToken token, string hid, List<dynamic> successfullySent)
        {
            foreach (var msg in replyMsgs)
            {
                ...
                if (payload != null)
                {
                    ...
                    {
                        await SendWSMessageAsync(currentWs, JsonConvert.SerializeObject(payload));
                    }
                    _totalMessages++;
                    ...
                    if (msg.content != null)
                    {
                        AddChatHistoryMessage("ai", msg.content.ToString());
                    }
                    else if (msg.meme != null)
                    {
                        AddChatHistoryMessage("ai", null, msg.meme.ToString());
                    }
                }
            }
        ...
        private static async Task SendWSMessageAsync(ClientWebSocket webSocket, string json) { if (webSocket?.State == WebSocketState.Open) await webSocket.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes(json)), WebSocketMessageType.Text, true, CancellationToken.None); }
        private static async Task SendKeepAliveAsync(ClientWebSocket webSocket) { while (webSocket.State == WebSocketState.Open) { await Task.Delay(_controlPanelConfig.WebsocketKeepAliveInterval); await SendWSMessageAsync(webSocket, "{\"action\":\"get_status\"}"); } }
        ...
        private static async Task SendStandardMessageAsync(string type, dynamic data = null)
        {
            ...
            if (currentWs != null && currentWs.State == WebSocketState.Open)
            {
                await SendWSMessageAsync(currentWs, json);
            }
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (context.Request.HttpMethod == "GET" && (path == "/" || path == "/index.html")) ServeControlPanelHtml(context);
                        else if (context.Request.HttpMethod == "GET" && path == "/api/config") ServeConfig(context);
                        else if (context.Request.HttpMethod == "POST" && path == "/api/config") await UpdateConfigAsync(context);
                        else if (context.Request.HttpMethod == "GET" && path == "/api/logs") ServeLogs(context);
                        else if (context.Request.HttpMethod == "DELETE" && path == "/api/logs") ClearLogs(context);
                        else if (context.Request.HttpMethod == "GET" && path == "/api/proxy") await ServeProxyAsync(context);
        ...
        private static void ClearContext()
        {
            ...
            SaveChatHistoryToDisk();
            BroadcastChatHistory();
            LogInfo("CONTROL_PANEL", "Chat history cleared by user request");
        }
        ...
        private static string GetQueryParameter(string q, string n) { if (string.IsNullOrEmpty(q)) return null; if (q.StartsWith("?")) q = q.Substring(1); var param = q.Split('&').Select(p => p.Split('=')).FirstOrDefault(parts => parts.Length == 2 && parts[0].Equals(n, StringComparison.OrdinalIgnoreCase)); if (param == null) return null; try { return Uri.UnescapeDataString(param[1]); } catch { return param[1]; } }
            ...
            lock (_configLock)
            {
                ...
                if (configData.websocketServerUri != null)
                {
                    ...
                    if (newUri != _controlPanelConfig.WebsocketServerUri)
                    {
                        ...
                        // Force close WebSocket connection immediately to trigger reconnection
                        if (_webSocket != null)
                        {
                            try
                            {
                                _webSocket.Dispose();
                                LogInfo("WS_CLIENT", "Force closed WebSocket connection for URI update");
                            }
                            catch { }
                        }
                    }
                }
                if (configData.websocketToken != null)
                {
                    string newToken = configData.websocketToken.ToString();
                    // Only update and reconnect if the token has actually changed
                    if (newToken != _controlPanelConfig.WebsocketToken)
                    {
                        _controlPanelConfig.WebsocketToken = newToken;
                        // Force close WebSocket connection immediately to trigger reconnection
                        if (_webSocket != null)
                        {
                            try
                            {
                                _webSocket.Dispose();
                                LogInfo("WS_CLIENT", "Force closed WebSocket connection for token update");
                            }
                            catch { }
                        }
                    }
                ...
                if (configData.roleCardsApiUrl != null) _controlPanelConfig.RoleCardsApiUrl = configData.roleCardsApiUrl.ToString();
        ...
        private static void AddLog(string l, string s, string m)
        {
            ...
            string safeMessage = WebUtility.HtmlEncode(m);
        ...
        private static void LogToFile(string sub, string m)
        {
            try
            {
                ...
                string dir = Path.Combine(Environment.CurrentDirectory, LOG_ROOT_FOLDER, sub);
        ...
        private static async Task ServeRoleCardsAsync(HttpListenerContext context)
        {
            try
            {
                string url = _controlPanelConfig.RoleCardsApiUrl;
            ...
            {
                LogError("ROLE_CARDS", "Error fetching role cards: " + ex.Message);
                context.Response.StatusCode = 500;
        ...
        private static async Task ServeRoleCardDetailsAsync(HttpListenerContext context)
        {
            try
            {
            ...
            {
                LogError("ROLE_CARDS", "Error fetching role card details: " + ex.Message);
                context.Response.StatusCode = 500;
                string error = JsonConvert.SerializeObject(new { error = "Internal server error" });
                byte[] buffer = Encoding.UTF8.GetBytes(error);
                context.Response.ContentType = "application/json";
        ...
        private static async Task DownloadEmojisAsync(List<string> emojiUrls)
        {
            try
            {
                ...
                if (!Directory.Exists(memeFolder))
                {
                    ...
                    LogInfo("ROLE_CARDS", "Created meme folder: " + memeFolder);
                }
                    ...
                    foreach (string emojiUrl in emojiUrls)
                    {
                        try
                        {
                            ...
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                LogInfo("ROLE_CARDS", "Downloaded emoji: " + fileName);
                            }
                            else
                            {
                                LogWarning("ROLE_CARDS", "Failed to download emoji: " + cleanUrl);
                            }
                        }
                        catch (Exception ex)
                        {
                            LogError("ROLE_CARDS", "Error downloading emoji " + emojiUrl + ": " + ex.Message);
                        }
                    }
                }
            }
            ...
            {
                LogError("ROLE_CARDS", "Error in DownloadEmojisAsync: " + ex.Message);
            }
        }

        private static async Task HandleUseRoleCardAsync(WebSocket webSocket, dynamic data)
        {
            try
            {
                // 解析角色卡数据
                string baseSystemPrompt = data?.baseSystemPrompt?.ToString();
                string reinforcementPrompt = data?.reinforcementPrompt?.ToString();
                dynamic emojisData = data?.roleCardAvailableEmojis;

                // 更新配置
                var newConfig = new ControlPanelConfig
                {
                    LlmModelName = _controlPanelConfig.LlmModelName,
                    LlmApiBaseUrl = _controlPanelConfig.LlmApiBaseUrl,
                    LlmApiKey = _controlPanelConfig.LlmApiKey,
                    LlmMaxTokens = _controlPanelConfig.LlmMaxTokens,
                    LlmTemperature = _controlPanelConfig.LlmTemperature,
                    LlmTopP = _controlPanelConfig.LlmTopP,
                    WebsocketServerUri = _controlPanelConfig.WebsocketServerUri,
                    WebsocketKeepAliveInterval = _controlPanelConfig.WebsocketKeepAliveInterval,
                    MaxContextRounds = _controlPanelConfig.MaxContextRounds,
                    TargetUserId = _controlPanelConfig.TargetUserId,
                    ActiveChatProbability = _controlPanelConfig.ActiveChatProbability,
                    MinSafeDelay = _controlPanelConfig.MinSafeDelay,
                    ProactiveChatEnabled = _controlPanelConfig.ProactiveChatEnabled,
                    ReminderEnabled = _controlPanelConfig.ReminderEnabled,
                    ReinforcementEnabled = _controlPanelConfig.ReinforcementEnabled,
                    IntentAnalysisEnabled = _controlPanelConfig.IntentAnalysisEnabled,
                    BaseSystemPrompt = baseSystemPrompt ?? _controlPanelConfig.BaseSystemPrompt,
                    IncompleteInputPrompt = _controlPanelConfig.IncompleteInputPrompt,
                    ReinforcementPrompt = reinforcementPrompt ?? _controlPanelConfig.ReinforcementPrompt
                };

                // 更新全局配置
                lock (_configLock)
                {
                    _controlPanelConfig = newConfig;
                    SaveConfig();
                }
                ...
                // 发送成功响应
                await webSocket.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(new WebSocketMessage { Type = "role_card_used", Data = "角色卡使用成功" }))), WebSocketMessageType.Text, true, CancellationToken.None);
                
                // 清空上下文，确保新角色卡的提示词立即生效
                ClearContext();
                
                // 广播更新后的配置给所有前端客户端
                BroadcastMessageToClients(new WebSocketMessage { Type = "config_updated", Data = _controlPanelConfig });
                
                LogInfo("ROLE_CARDS", "Role card used successfully");
                LogInfo("ROLE_CARDS", "Context cleared for new role card");
                LogInfo("ROLE_CARDS", "Config updated and broadcast to clients");
            }
            catch (Exception ex)
            {
                LogError("ROLE_CARDS", "Error handling use_role_card: " + ex.Message);
                await webSocket.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(new WebSocketMessage { Type = "role_card_error", Data = "角色卡使用失败: " + ex.Message }))), WebSocketMessageType.Text, true, CancellationToken.None);
            }
        }

        private static async Task ServeProxyAsync(HttpListenerContext context)
        {
            try
            {
                string query = context.Request.Url.Query;
                ...
                switch (action)
                {
                    ...
                    default:
                        context.Response.StatusCode = 400;
                        string error = "Missing or invalid action parameter";
                        byte[] buffer = Encoding.UTF8.GetBytes(error);
            ...
            {
                LogError("PROXY", "Error handling proxy request: " + ex.Message);
                context.Response.StatusCode = 500;
        ...
        private static async Task ServeProxyImageAsync(HttpListenerContext context)
        {
            try
            {
                string query = context.Request.Url.Query;
                ...
                using (var client = new HttpClient())
                {
                    ...
                    {
                        context.Response.StatusCode = 500;
                        string error = "Failed to fetch image";
                        byte[] buffer = Encoding.UTF8.GetBytes(error);
                        context.Response.ContentType = "text/plain";
                        context.Response.OutputStream.Write(buffer, 0, buffer.Length);
                    }
            ...
            {
                LogError("PROXY", "Error proxying image: " + ex.Message);
                context.Response.StatusCode = 500;
                string error = "Internal server error";
                byte[] buffer = Encoding.UTF8.GetBytes(error);
                context.Response.ContentType = "text/plain";
        ...
        private static async Task ServeMemeAsync(HttpListenerContext context)
        {
            try
            {
                string query = context.Request.Url.Query;
                ...
                {
                    context.Response.StatusCode = 400;
                    string error = "Missing name parameter";
                    byte[] buffer = Encoding.UTF8.GetBytes(error);
                    context.Response.ContentType = "text/plain";
                    context.Response.OutputStream.Write(buffer, 0, buffer.Length);
                    context.Response.Close();
            ...
            {
                LogError("PROXY", "Error serving meme: " + ex.Message);
                context.Response.StatusCode = 500;
                string error = "Internal server error";
                byte[] buffer = Encoding.UTF8.GetBytes(error);
                context.Response.ContentType = "text/plain";
