...
namespace AI_Chat.Services
{
    public class EmbeddingService
    {
        ...
        public EmbeddingService(ConfigManager configManager = null, RequestRateLimiter requestRateLimiter = null)
        {
            ...
            _localEmbeddingService = LocalEmbeddingService.Instance;
        }
        ...
        public float[] GenerateEmbedding(string text)
        {
            ...
            try
            {
                bool useLocalEmbedding = _configManager?.Config.UseLocalEmbedding ?? false;
                ...
                {
                    embedding = GenerateApiEmbedding(text);
                }

                if (embedding == null)
                {
                    Logger.LogError("EMBEDDING", "Failed to generate embedding, returning null");
                }
        ...
        private float[] GenerateApiEmbedding(string text)
        {
                ...
                if (_openAIService != null)
                {
                    ...
                    if (result.Successful)
                    {
                        ...
                        if (embedding != null && embedding.Count > 0)
                        {
                            ...
                            Logger.LogInfo("EMBEDDING", $"Successfully generated API embedding for text, dimension: {floatArray.Length}");
                            return floatArray;
                        }
                    }
                    else
                    {
                        Logger.LogError("EMBEDDING", $"Failed to generate API embedding: {result.Error?.Message}");
                    }
                }
            }
        ...
        {
            if (!_localEmbeddingService.IsInitialized)
            {
                if (!_localEmbeddingService.Initialize())
                {
                    Logger.LogError("EMBEDDING", "Failed to initialize local embedding service");
                    return null;
                }
            }

            var embedding = _localEmbeddingService.GenerateEmbedding(text);
            if (embedding != null)
            {
                Logger.LogInfo("EMBEDDING", $"Successfully generated local embedding for text, dimension: {embedding.Length}");
                return embedding;
            }
        ...
        private async Task<float[]> DoGenerateEmbeddingAsync(string text)
        {
            ...
            try
            {
                ...
                if (useLocalEmbedding)
                {
                    embedding = await Task.Run(() => GenerateLocalEmbedding(text));
                }
                else
                {
                    embedding = await GenerateApiEmbeddingAsync(text);
                }
        ...
        private async Task<float[]> GenerateApiEmbeddingAsync(string text)
        {
            var apiKey = GetEmbeddingApiKey();
            var baseUrl = GetEmbeddingApiBaseUrl();
            var modelName = GetEmbeddingModelName();
