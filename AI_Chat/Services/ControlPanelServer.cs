...
namespace AI_Chat.Services
{
    public class ControlPanelServer
    {
        ...
        /// <summary>
        /// 发送响应消息，自动添加 replyTo 字段
        /// </summary>
        private async Task SendResponseAsync(WebSocket webSocket, string type, dynamic data, string replyTo)
        {
            ...
            await SendSafeAsync(webSocket, JsonConvert.SerializeObject(response));
        }
        ...
        private async Task ProcessWebSocketMessageAsync(WebSocket webSocket, string json)
        {
            try
            {
                ...
                string messageId = message.Id; // 获取请求消息的ID用于追踪
                ...
                switch (message.Type)
                {
                    ...
                    case "get_chat_history":
                        ...
                        break;
                    case "select_user":
                        await HandleSelectUserAsync(webSocket, message.Data, messageId);
                        break;
                    case "get_users":
                        await SendUsersListAsync(webSocket, messageId);
                        break;
                    case "add_allowed_user":
                        await HandleAddAllowedUserAsync(message.Data, messageId);
                        break;
                    case "remove_allowed_user":
                        await HandleRemoveAllowedUserAsync(message.Data, messageId);
                        break;
                    case "add_allowed_group":
                        await HandleAddAllowedGroupAsync(message.Data, messageId);
                        break;
                    case "remove_allowed_group":
                        await HandleRemoveAllowedGroupAsync(message.Data, messageId);
                        break;
                    case "get_user_config":
                        await HandleGetUserConfigAsync(webSocket, message.Data, messageId);
                        break;
                    case "update_user_config":
                        await HandleUpdateUserConfigAsync(webSocket, message.Data, messageId);
                        break;
                    case "reset_user_config":
                        await HandleResetUserConfigAsync(webSocket, message.Data, messageId);
                        break;
                    ...
                    case "get_vector_entries":
                        await HandleGetVectorEntriesAsync(webSocket, message.Data, messageId);
                        break;
                    case "search_vectors":
                        await HandleSearchVectorsAsync(webSocket, message.Data, messageId);
                        break;
                    case "delete_vector_entry":
                        await HandleDeleteVectorEntryAsync(webSocket, message.Data, messageId);
                        break;
                    case "clear_vectors":
                        await HandleClearVectorsAsync(webSocket, message.Data, messageId);
                        break;
                    case "regenerate_vectors":
                        await HandleRegenerateVectorsAsync(webSocket, message.Data, messageId);
                        break;
                    case "save_vector_db_settings":
                        await HandleSaveVectorDbSettingsAsync(webSocket, message.Data, messageId);
                        break;
                    default:
        ...
        private async Task HandleSelectUserAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            try
            {
                ...
                if (_configManager.Config.AllowedUserIds.Contains(userId))
                {
                    ...
                    await SendResponseAsync(webSocket, "user_selecting", new { userId = userId }, replyTo);
        ...
        private async Task SendUsersListAsync(WebSocket webSocket, string replyTo = null)
        {
            try
            {
                ...
                await SendResponseAsync(webSocket, "users_list", new { users = users, groups = groups }, replyTo);
            }
            catch (Exception ex)
            {
        ...
        private async Task HandleAddAllowedUserAsync(dynamic data, string replyTo = null)
        {
            try
            {
                long userId = (long)data.userId;
                ...
                BroadcastMessageToClients(new WebSocketMessage { Type = "config_updated", Data = _configManager.Config, ReplyTo = replyTo });
            }
            catch (Exception ex)
            {
        ...
        private async Task HandleRemoveAllowedUserAsync(dynamic data, string replyTo = null)
        {
            try
            {
                long userId = (long)data.userId;
        ...
        private async Task HandleAddAllowedGroupAsync(dynamic data, string replyTo = null)
        {
            try
            {
        ...
        private async Task HandleRemoveAllowedGroupAsync(dynamic data, string replyTo = null)
        {
            try
            {
                long groupId = (long)data.groupId;
        ...
        private async Task HandleGetUserConfigAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            try
            {
                ...
                await SendResponseAsync(webSocket, "user_config", userConfig, replyTo);
            }
            catch (Exception ex)
            {
        ...
        private async Task HandleUpdateUserConfigAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            try
            {
                long userId = _selectedUserId;
                ...
                BroadcastMessageToClients(new WebSocketMessage { Type = "user_config_updated", Data = userConfig, ReplyTo = replyTo });
            }
            catch (Exception ex)
            {
        ...
        private async Task HandleResetUserConfigAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            try
            {
                long userId = _selectedUserId;
        ...
        private async Task SendInitialDataAsync(WebSocket webSocket)
        {
            try
            {
                ...
                // 使用分页数据，只加载最新的20条
                var chatHistoryData = new
                {
                    messages = chatHistoryManager?.GetLatestMessages(20) ?? new List<ChatMessage>(),
                    hasMore = chatHistoryManager?.GetMessageCount() > 20,
                    totalCount = chatHistoryManager?.GetMessageCount() ?? 0
                };
        ...
        private async Task BroadcastChatHistoryAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            ...
            if (data?.limit != null)
            {
                ...
                limit = Math.Clamp(limit, 1, 100); // 限制每次最多100条
            }
        ...
        private async Task HandleGetVectorEntriesAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            try
            {
                long userId = _selectedUserId;
                ...
                if (userId == 0)
                {
                    await SendResponseAsync(webSocket, "vector_entries", 
        ...
        private async Task HandleSearchVectorsAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            try
            {
                long userId = data?.userId != null ? (long)data.userId : _selectedUserId;
                ...
                {
                    await SendResponseAsync(webSocket, "vector_search_results", new List<VectorEntry>(), replyTo);
                    return;
                }
                ...
                await SendResponseAsync(webSocket, "vector_search_results", results, replyTo);
            }
            catch (Exception ex)
            {
        ...
        private async Task HandleDeleteVectorEntryAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            try
            {
                long userId = _selectedUserId;
                ...
                if (contextManager == null)
                {
                    await SendResponseAsync(webSocket, "error", new { message = "Vector context not available" }, replyTo);
                    return;
                }
                contextManager.DeleteVectorEntry(entryId);
                
                await SendResponseAsync(webSocket, "vector_entry_deleted", new { id = entryId }, replyTo);
        ...
        private async Task HandleClearVectorsAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            try
            {
                long userId = _selectedUserId;
                ...
                await SendResponseAsync(webSocket, "vectors_cleared", new { userId = userId }, replyTo);
        ...
        private async Task HandleRegenerateVectorsAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            try
            {
                long userId = _selectedUserId;
                ...
                await SendResponseAsync(webSocket, "vectors_regenerated", new { userId = userId }, replyTo);
        ...
        private async Task HandleSaveVectorDbSettingsAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            try
            {
                ...
                await SendResponseAsync(webSocket, "vector_db_settings_saved", new { success = true }, replyTo);
