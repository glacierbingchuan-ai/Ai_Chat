...
namespace AI_Chat.Services
{
    public class ConfigManager
    {
            ...
            try
            {
                if (File.Exists(AppConstants.CONFIG_FILE_PATH))
                {
                    ...
                    if (loadedConfig != null)
                    {
                        ...
                        // 如果配置中选择了本地嵌入，自动预加载模型
                        if (_config.UseLocalEmbedding && LocalEmbeddingService.IsModelExists())
                        {
                            _ = Task.Run(() =>
                            {
                                try
                                {
                                    Logger.LogInfo("CONFIG", "配置中已启用本地嵌入，开始预加载模型...");
                                    var localEmbeddingService = LocalEmbeddingService.Instance;
                                    bool success = localEmbeddingService.Initialize();
                                    if (success)
                                    {
                                        Logger.LogInfo("CONFIG", "本地嵌入模型预加载成功");
                                    }
                                    else
                                    {
                                        Logger.LogError("CONFIG", "本地嵌入模型预加载失败");
                                    }
                                }
                                catch (Exception ex)
                                {
                                    Logger.LogError("CONFIG", $"启动时预加载本地嵌入模型失败: {ex.Message}", ex);
                                }
                            });
                        }
                    }
        ...
        public bool UpdateConfig(dynamic configData, out string errorMessage)
        {
            errorMessage = null;
            lock (_configLock)
            {
                ...
                // 处理本地 Embedding 配置
                bool useLocalEmbeddingChanged = false;
                bool newUseLocalEmbedding = _config.UseLocalEmbedding;
                if (configData.useLocalEmbedding != null)
                {
                    newUseLocalEmbedding = (bool)configData.useLocalEmbedding;
                    useLocalEmbeddingChanged = newUseLocalEmbedding != _config.UseLocalEmbedding;
                }
                if (configData.UseLocalEmbedding != null)
                {
                    newUseLocalEmbedding = (bool)configData.UseLocalEmbedding;
                ...
                // 验证：如果用户选择本地运行但未初始化环境，不允许保存
                // 注意：只要选择了本地嵌入且模型不存在就报错，不管配置是否改变
                if (newUseLocalEmbedding && !LocalEmbeddingService.IsModelExists())
                {
                    errorMessage = "本地嵌入模型未初始化，请先下载模型";
                    return false;
                }

                // 应用配置
                _config.UseLocalEmbedding = newUseLocalEmbedding;
                if (configData.localEmbeddingModelPath != null) _config.LocalEmbeddingModelPath = configData.localEmbeddingModelPath.ToString();
                if (configData.LocalEmbeddingModelPath != null) _config.LocalEmbeddingModelPath = configData.LocalEmbeddingModelPath.ToString();

                SaveConfig();

                // 根据配置自动加载或卸载本地嵌入模型
                if (useLocalEmbeddingChanged)
                {
                    _ = Task.Run(() =>
                    {
                        try
                        {
                            var localEmbeddingService = LocalEmbeddingService.Instance;
                            if (newUseLocalEmbedding)
                            {
                                // 用户选择了本地嵌入，预加载模型
                                if (LocalEmbeddingService.IsModelExists())
                                {
                                    Logger.LogInfo("CONFIG", "用户选择本地嵌入，开始预加载模型...");
                                    bool success = localEmbeddingService.Initialize();
                                    if (success)
                                    {
                                        Logger.LogInfo("CONFIG", "本地嵌入模型预加载成功");
                                ...
                                {
                                    Logger.LogWarning("CONFIG", "用户选择本地嵌入，但模型文件不存在，跳过预加载");
                                }
                            }
                            else
                            {
                                ...
                                {
                                    Logger.LogInfo("CONFIG", "用户取消本地嵌入，卸载模型...");
                                    localEmbeddingService.Uninitialize();
                                    Logger.LogInfo("CONFIG", "本地嵌入模型已卸载");
                                }
                            }
                        }
                        catch (Exception ex)
                        {
                            Logger.LogError("CONFIG", $"处理本地嵌入模型加载/卸载时出错: {ex.Message}", ex);
                        }
                    });
                }
