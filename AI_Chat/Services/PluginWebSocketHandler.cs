...
namespace AI_Chat.Plugins
{
    ...
    public class PluginWebSocketHandler
    {
        ...
        public PluginWebSocketHandler(PluginManager pluginManager, WebSocketManager wsManager = null)
        {
        ...
        private async Task HandleGetPluginPermissionsAsync(WebSocket webSocket, dynamic data, string replyTo = null)
        {
            ...
            try
            {
                ...
                // 获取插件能力列表
                var capabilities = plugin.GetCapabilities() ?? new List<PluginCapability>();
                var capabilityList = capabilities.Select(c => new
                {
                    Name = c.Name,
                    Description = c.Description,
                    MethodName = c.MethodName,
                    Parameters = c.Parameters?.Select(p => (object)new
                    {
                        p.Name,
                        p.Type,
                        p.Description,
        ...
        /// <summary>
        /// 发送消息 - 使用 WebSocketManager 确保线程安全
        /// </summary>
        private async Task SendMessageAsync(WebSocket webSocket, string type, object data, string replyTo = null)
        {
            ...
            await _wsManager.SendServerMessageAsync(webSocket, message);
        }
