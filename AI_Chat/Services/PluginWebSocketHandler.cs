...
namespace AI_Chat.Plugins
{
    ...
    public class PluginWebSocketHandler
    {
        ...
        public PluginWebSocketHandler(PluginManager pluginManager, PluginVirtualizationManager virtualizationManager = null)
        {
            ...
            try
            {
                switch (messageType)
                {
                    ...
                    case "get_plugin_permissions":
                        ...
                        break;
                    case "approve_plugin":
                        await HandleApprovePluginAsync(webSocket, data);
                        break;
                    default:
        ...
        private async Task HandleStartPluginAsync(WebSocket webSocket, dynamic data)
        {
            ...
            // 检查插件是否处于等待确认状态
            if (plugin?.State == PluginState.PendingApproval)
            {
                await SendMessageAsync(webSocket, "plugin_start_failed", new Dictionary<string, object>
                {
                    { "pluginId", pluginId },
                    { "pluginName", plugin.Name },
                    { "success", false },
                    { "state", plugin.State.ToString() },
                    { "message", "插件需要用户确认后才能启动" },
            ...
            plugin = _pluginManager.GetPluginInfo(pluginId);
        ...
        private async Task HandleUnloadPluginAsync(WebSocket webSocket, dynamic data)
        {
            ...
            if (success && _virtualizationManager != null)
            {
                _virtualizationManager.RemoveVirtualization(pluginId);
                _virtualizationManager.ClearPluginData(pluginId);
                Logger.LogInfo("PLUGIN_WS", $"Virtualization removed and data cleared for unloaded plugin: {pluginId}");
            }
        ...
        private async Task HandleSetPluginConfigAsync(WebSocket webSocket, dynamic data)
        {
            ...
            {
                await SendErrorAsync(webSocket, $"Configuration update failed: {ex.Message}");
            }
        }
        ...
        /// 用户确认加载不支持沙箱的插件
        /// </summary>
        private async Task HandleApprovePluginAsync(WebSocket webSocket, dynamic data)
        {
            string pluginId = data?.pluginId?.ToString();
            if (string.IsNullOrEmpty(pluginId))
            {
            ...
            if (plugin.State != PluginState.PendingApproval)
            {
                await SendErrorAsync(webSocket, $"插件 {pluginId} 不处于等待确认状态");
                return;
            }

            bool success = _pluginManager.ApproveAndLoadPlugin(pluginId);
            
            // 重新获取插件信息以获取最新状态
            plugin = _pluginManager.GetPluginInfo(pluginId);

            await SendMessageAsync(webSocket, "plugin_approved", new Dictionary<string, object>
            {
                { "pluginId", pluginId },
                { "pluginName", plugin?.Name ?? pluginId },
                { "success", success },
                { "state", plugin?.State.ToString() },
                { "message", success ? "插件已确认并加载成功" : "插件确认加载失败" }
            });
        }
