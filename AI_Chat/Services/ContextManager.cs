    ...
    public class ContextManager : IContextManager
    {
        ...
        public List<Message> GetContextForPrompt(string query, int maxRecentMessages = 10, float similarityThreshold = 0.2f)
        {
            ...
            lock (_contextLock)
            {
                ...
                var basePrompt = _context.FirstOrDefault(m => m.Role == "system" && !m.Content.StartsWith("对话总结：") && !m.Content.Contains(AppConstants.TAG_FORMAT_ERROR));
                if (basePrompt != null)
                {
                ...
                // 4. 添加格式错误纠正消息（放在最后，让模型能看到之前的错误）
                var formatErrorMsgs = _context.Where(m => m.Role == "system" && m.Content.Contains(AppConstants.TAG_FORMAT_ERROR)).ToList();
                context.AddRange(formatErrorMsgs);
            }
