    ...
    public class ChatHistoryManager
    {
        ...
        public ChatHistoryManager(long userId = 0)
        {
            ...
            if (userId > 0)
            {
                ...
                _chatHistoryPath = Path.Combine(userDir, "chat_history.json");
            }
            else
            {
                _chatHistoryPath = AppConstants.CHAT_HISTORY_PATH;
            }
        }
        ...
        public void LoadChatHistoryFromDisk()
        {
            try
            {
                ...
                {
                    string json = File.ReadAllText(_chatHistoryPath);
                    ...
                    if (savedChatHistory != null && savedChatHistory.Count > 0)
                    {
                        ...
                        Logger.LogInfo("PERSISTENCE", $"Loaded {savedChatHistory.Count} chat messages from local disk (User: {_userId})");
                        return;
                    }
                }
                Logger.LogInfo("PERSISTENCE", $"No chat history found or file is empty, initializing new chat history (User: {_userId})");
            }
            catch (Exception ex)
            {
        ...
        public void SaveChatHistoryToDisk()
        {
            ...
            {
                Logger.LogError("PERSISTENCE", $"Failed to save chat history (User: {_userId}): {ex.Message}");
            }
        }
